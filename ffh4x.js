...
function aimHeadLock(enemy, weapon = "default") {
...
function flickHeadshot(enemy, weapon = "default") {
...
function recoilCompensation(weapon = "default") {
...
(async function () {
  ...
  const enemy = req.enemy || {
    position: { x: 22, y: 4, z: 0 },
    velocity: { x: 0.12, y: 0.03, z: 0 },
    height: 1.8
  };
  const weapon = req.weapon || "default";
...
// === AimLock Engine Script cho Shadowrocket ===
...
const aimConfig = {
  ...
  structured_tracking: {
    ...
    rotation: { x: 0.0258174837, y: -0.08611039, z: -0.1402113, w: 0.9860321 },
    scale: { x: 0.99999994, y: 1.00000012, z: 1.0 },
    center_reference: { x: -0.015870847, y: 1.4875782, z: -0.0072678495 },
    follow_gain: 1.65,
    rotation_lock_strength: 1.1,
    snap_back_enabled: true,
    snap_back_speed: 0.95
  },

  velocity_tracking: {
    enabled: true,
    dynamic_direction: true,
    bone_source: "bone_Head",
    adapt_x: true,
    adapt_y: true,
    adapt_z: true,
    multiplier: 1.65,
    clamp: {x: 0.9, y: 0.5, z: 0.55  }
  },
...
// 🎯 Gửi kết quả config về ứng dụng
$done({ config: aimConfig });

function adjustAimAfterPull(currentPos, targetHeadPos, armorZonePos, pullDistance, boneInfo = {}) {
  // === 1. Lấy offset từ bone head nếu có ===
  const offset = boneInfo.offset || { x: 0, y: 0, z: 0 };
  const targetPos = {
    x: targetHeadPos.x + offset.x,
    y: targetHeadPos.y + offset.y,
    z: targetHeadPos.z + offset.z
  };

  // === 2. Tính khoảng cách ===
  const distToHead = Math.hypot(currentPos.x - targetPos.x, currentPos.y - targetPos.y);
  const distToArmor = Math.hypot(currentPos.x - armorZonePos.x, currentPos.y - armorZonePos.y);

  // === 3. Kéo nhẹ về armor nếu gần hơn 70% so với head ===
  if (distToArmor < distToHead * 0.7) {
    currentPos.x += (armorZonePos.x - currentPos.x) * 0.3;
    currentPos.y += (armorZonePos.y - currentPos.y) * 0.3;
  }

  // === 4. Kéo theo easing về head ===
  const t = Math.min(1, pullDistance / 50);
  const easeOutQuad = (x) => 1 - (1 - x) * (1 - x);
  const easeFactor = easeOutQuad(t);

  currentPos.x += (targetPos.x - currentPos.x) * easeFactor * 0.6;
  currentPos.y += (targetPos.y - currentPos.y) * easeFactor * 0.6;
  if ("z" in currentPos && "z" in targetPos) {
    currentPos.z += (targetPos.z - currentPos.z) * easeFactor * 0.6;
  }

  // === 5. Nếu gần head → kéo mạnh hơn theo radius ===
  const radius = boneInfo.radius || 6.5;
  const distToBone = Math.hypot(currentPos.x - targetPos.x, currentPos.y - targetPos.y);

  if (distToBone < radius) {
    const heavyPullFactor = (radius - distToBone) / radius;
    currentPos.x += (targetPos.x - currentPos.x) * heavyPullFactor * 0.8;
    currentPos.y += (targetPos.y - currentPos.y) * heavyPullFactor * 0.8;
    if ("z" in currentPos && "z" in targetPos) {
      currentPos.z += (targetPos.z - currentPos.z) * heavyPullFactor * 0.8;
    }
  }
